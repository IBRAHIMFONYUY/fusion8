...
export const FirebaseProvider: React.FC<FirebaseProviderProps> = ({
  ...
  // Only block render when a signed-in user's profile is still loading.
  // This prevents role/permission flash on protected routes while still allowing
  // public pages (landing, courses, projects) to render immediately without delay.
  if (user && isProfileLoading) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-background z-50">
        <Loader2 className="h-10 w-10 text-accent animate-spin mb-4" />
        <h2 className="text-lg font-bold font-headline tracking-tight text-primary">FUSION8</h2>
        <p className="text-muted-foreground text-xs font-medium tracking-widest mt-1">Synchronizing...</p>
      </div>
    );
  }
