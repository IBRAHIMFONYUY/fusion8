...
export default function TeacherGradebookPage() {
    ...
    const handleGradeSave = async (submissionId: string, grade: string, feedback: string, passed: boolean) => {
        try {
    ...
    <>
              ...
              <TableBody>
                {submissions.map(sub => (
                    <TableRow key={sub.id} className="hover:bg-accent/5 transition-colors">
                      ...
                      <TableCell>
                        <p className="text-sm font-medium">{sub.lessonTitle || sub.assignmentTitle || 'Untitled'}</p>
                        <p className="text-[10px] text-muted-foreground font-mono uppercase">ID: {sub.id.slice(0,8)}</p>
                      </TableCell>
                      <TableCell>
                        <Badge variant={sub.status === 'passed' ? 'default' : sub.status === 'needs_revision' ? 'destructive' : 'secondary'}
                          className={sub.status === 'passed' ? 'bg-green-600' : ''}>
                          {sub.status === 'passed' ? 'Passed' : sub.status === 'needs_revision' ? 'Needs Revision' : 'Pending'}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {(sub.externalUrl || sub.submissionLink) ? (
                          <a href={sub.externalUrl || sub.submissionLink} target="_blank" rel="noreferrer" className="text-accent hover:underline text-xs flex items-center gap-1 font-bold">
      ...
      {selectedSubmission && (
        <GradeSubmissionDialog
          ...
          onGradeSave={(_id: string | number, g: string, f: string, passed: boolean) => { handleGradeSave(selectedSubmission.id, g, f, passed); }}
        />
      )}
    </>
  )
