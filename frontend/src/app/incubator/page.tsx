...
const GRADES = [
  ...
  { grade: 'B',  range: '55–74',  label: 'Incubation Ready', desc: 'Enter Innovation Incubator', color: 'text-foreground border-black/[0.12] bg-card' },
  { grade: 'C',  range: '35–54',  label: 'Prototype Stage', desc: 'Re-enter Academy for skills gap', color: 'text-foreground/60 border-black/[0.08] bg-card/50' },
  { grade: 'D',  range: '0–34',   label: 'Idea Stage', desc: 'Needs fundamental development', color: 'text-foreground/35 border-black/[0.06] bg-transparent' },
];
      ...
      <section className="bg-[#F3F4F6] border-b border-black/[0.06]">
        <div className="container mx-auto px-4 py-20">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
            {STAGES.map((s, i) => (
              <div key={s.n} className="bg-card border border-black/[0.06] hover:border-accent/25 rounded-2xl p-6 transition-all group">
                ...
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(100,116,139,0.75)' }}>{s.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-background">
        <div className="container mx-auto px-4 py-20 md:py-28">
          <div className="text-center mb-14">
            ...
            <p className="text-base max-w-2xl mx-auto" style={{ color: 'rgba(100,116,139,0.80)' }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto mb-14">
            {DIMENSIONS.map((d, i) => {
              const Icon = d.icon;
              return (
                <div key={d.title} className="bg-card border border-black/[0.06] hover:border-accent/20 rounded-2xl p-6 transition-all">
                  ...
                  <p className="text-xs leading-relaxed" style={{ color: 'rgba(100,116,139,0.80)' }}>{d.desc}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="max-w-3xl mx-auto">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-center mb-6" style={{ color: 'rgba(100,116,139,0.55)' }}>Overall Grades</p>
              ...
              {GRADES.map(g => (
                <div key={g.grade} className={`flex items-center gap-5 p-4 rounded-xl border ${g.color} transition-all`}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'rgba(100,116,139,0.70)' }}>{g.desc}</p>
                  </div>
                  <div className="text-xs font-mono font-bold shrink-0" style={{ color: 'rgba(100,116,139,0.50)' }}>{g.range}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 py-20">
          <div className="grid md:grid-cols-2 gap-12 items-center max-w-4xl mx-auto">
            ...
            <div className="bg-card border border-accent/20 rounded-2xl p-8 text-center">
              ...
              <p className="text-sm mb-6" style={{ color: 'rgba(100,116,139,0.80)' }}>Standard incubation duration</p>
              ...
              <div className="space-y-3 text-sm text-left">
                {['Open to Fusion 8 Academy graduates', 'Grade B or above at Demo Day', 'Working prototype required', 'Team of 1–4 people'].map(req => (
                  <div key={req} className="flex items-start gap-2.5" style={{ color: 'rgba(100,116,139,0.85)' }}>
      ...
      <section className="bg-accent relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-20 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button asChild size="lg" variant="outline" className="border-black/[0.15] text-white hover:bg-black/[0.05] h-12 px-7 rounded-xl font-semibold">
