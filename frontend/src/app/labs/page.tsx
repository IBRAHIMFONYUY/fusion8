    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <section className="relative pt-32 pb-20 grid-texture border-b border-black/[0.06]">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button asChild size="lg" variant="outline" className="border-black/[0.12] text-foreground hover:bg-black/[0.03] h-12 px-7 rounded-xl font-semibold">
          ...
          <div className="max-w-3xl mx-auto grid sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={s.l} className="bg-card border border-black/[0.06] rounded-2xl p-5 text-center">
                ...
                <div className="text-[10px]" style={{ color: 'rgba(100,116,139,0.65)' }}>{s.sub}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 py-20 md:py-28">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
            {LAB_SERVICES.map(s => {
              ...
              return (
                <div key={s.title} className={`bg-card border rounded-2xl p-6 hover:border-accent/25 transition-all ${isAvailable ? 'border-accent/15' : 'border-black/[0.06]'}`}>
                  <div className="flex items-center justify-between mb-4">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isAvailable ? 'bg-accent/15' : 'bg-black/[0.03]'}`}>
                      <Icon className={`h-4.5 w-4.5 ${isAvailable ? 'text-accent' : 'text-foreground/30'}`} />
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${isAvailable ? 'bg-accent text-white' : 'bg-black/[0.04]'}`} style={{ color: isAvailable ? undefined : 'rgba(246,245,242,0.35)' }}>
                      {s.status}
                    </span>
                  </div>
                  <h3 className="text-sm font-black text-foreground mb-1.5">{s.title}</h3>
                  <p className="text-xs leading-relaxed" style={{ color: 'rgba(100,116,139,0.75)' }}>{s.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="bg-[#F3F4F6] border-y border-black/[0.06]">
        <div className="container mx-auto px-4 py-20">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
            {PRICING.map((p, i) => (
              <div key={p.label} className={`bg-card border rounded-2xl p-6 text-center hover:border-accent/25 transition-all ${i === 0 ? 'border-accent/25' : 'border-black/[0.06]'}`}>
                ...
                <p className="text-[10px] leading-relaxed" style={{ color: 'rgba(100,116,139,0.65)' }}>{p.note}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-background">
        <div className="container mx-auto px-4 py-20 max-w-3xl text-center">
          ...
          <p className="text-base leading-relaxed mb-10" style={{ color: 'rgba(100,116,139,0.85)' }}>
