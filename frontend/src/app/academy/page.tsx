      ...
      <section className="relative pt-32 pb-20 border-b border-white/[0.06] overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button asChild size="lg" variant="outline" className="border-white/20 text-foreground hover:bg-white/5 rounded-xl h-13 px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
            {[
              ...
              <div key={s.l} className="bg-card border border-white/[0.07] rounded-2xl p-5 text-center">
                ...
                <div className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'rgba(246,245,242,0.45)' }}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={week.week} className="border rounded-2xl overflow-hidden transition-all hover:border-accent/25" style={{ background: PHASE_COLORS[week.phase] || 'rgba(17,24,32,1)', borderColor: i === 6 ? 'rgba(228,77,40,0.35)' : 'rgba(246,245,242,0.07)' }}>
                <div className="flex flex-col md:flex-row md:items-start gap-4 p-6">
                  <div className="shrink-0 md:w-28">
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-wider mt-1" style={{ color: 'rgba(246,245,242,0.30)' }}>{week.phase}</p>
                  </div>
                  <div className="flex-1">
                    <h3 className={`text-base font-black mb-3 ${i === 6 ? 'text-accent' : 'text-foreground'}`}>{week.title}</h3>
                    <div className="flex flex-wrap gap-2">
                      {week.topics.map(t => (
                        <span key={t} className="text-xs px-3 py-1.5 rounded-full border" style={{ color: 'rgba(246,245,242,0.65)', borderColor: 'rgba(246,245,242,0.10)', background: 'rgba(246,245,242,0.04)' }}>
                          {t}
                        </span>
                      ))}
                    </div>
      ...
      <section className="bg-background">
        <div className="container mx-auto px-4 py-20">
          <div className="text-center mb-12">
            ...
            <p className="text-base max-w-xl mx-auto" style={{ color: 'rgba(246,245,242,0.48)' }}>Fusion 8 will expand into 14 industry tracks by 2030.</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-w-4xl mx-auto">
            {TRACKS.map(({ icon: Icon, label, status }) => (
              <div key={label} className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${status === 'now' ? 'border-accent/25' : 'bg-card border-white/[0.06]'}`} style={{ background: status === 'now' ? 'rgba(228,77,40,0.07)' : undefined }}>
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${status === 'now' ? 'bg-accent/15' : 'bg-white/[0.04]'}`}>
                  <Icon className={`h-3.5 w-3.5 ${status === 'now' ? 'text-accent' : 'text-foreground/30'}`} />
                </div>
                <div>
                  <p className="text-xs font-semibold leading-tight" style={{ color: status === 'now' ? 'rgba(246,245,242,0.88)' : 'rgba(246,245,242,0.48)' }}>{label}</p>
                  <p className="text-[9px] font-black uppercase tracking-wider mt-0.5" style={{ color: status === 'now' ? 'rgba(228,77,40,0.9)' : 'rgba(246,245,242,0.25)' }}>
